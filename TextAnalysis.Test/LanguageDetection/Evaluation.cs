...
internal class MinMaxAvg {
	...
	public Double Avg = 0;
	public Int64 Values = 0;

	public void Add(Double value) {
		Min = Math.Min(Min, value);
		Max = Math.Max(Max, value);
		Avg = (Avg * Values + value) / (Values + 1);
		++Values;
	}

	public void Add(MinMaxAvg mma) {
		Min = Math.Min(Min, mma.Min);
		Max = Math.Max(Max, mma.Max);
		Avg = (Avg * Values + mma.Avg * mma.Values) / (Values + mma.Values);
		Values += mma.Values;
	}
}
...
internal class LanguageTestStats {
	...
	public MinMaxAvg PositiveRange = new();
	public MinMaxAvg NegativeRange = new();
	...
	public void Add(LanguagePrediction languagePrediction, Language correctLanguage) {
		...
		if (languagePrediction.Language == correctLanguage) {
			++Positive;
			PositiveRange.Add(languagePrediction.Confidence);
		} else if (languagePrediction.Language == SpecialLanguageCodes.Undetermined)
			++Unknown;
		else {
			++Negative;
			NegativeRange.Add(languagePrediction.Confidence);
		}
	}
	...
	public void Add(String category, LanguageTestStats languageTestStats) {
		...
		PositiveRange.Add(languageTestStats.PositiveRange);
		NegativeRange.Add(languageTestStats.NegativeRange);
	}
...
public class Evaluation : ATest {
	...
	public void TestDetector<T>(String name, Func<T> creator, Func<T, String, LanguagePrediction> detect) where T : class {
		PerformanceHelper.EstimateObjectSize(name, 0, 1, Console.Out, 0, _ => creator());
